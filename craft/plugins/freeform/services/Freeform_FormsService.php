...
class Freeform_FormsService extends BaseApplicationComponent implements FormHandlerInterface
{
    ...
    {
        $event = new Event($this, ['form' => $form]);
        $this->raiseEvent(FreeformPlugin::EVENT_BEFORE_SUBMIT, $event);
     ...
     * @inheritDoc
     */
    public function onAfterSubmit(Form $form, Freeform_SubmissionModel $model = null)
    {
        $event = new Event($this, ['form' => $form, 'submission' => $model]);
        $this->raiseEvent(FreeformPlugin::EVENT_AFTER_SUBMIT, $event);
    }
    ...
    public function addScriptsToPage(Form $form)
    {
        ...
        if ($this->getSettingsService()->isFormSubmitDisable()) {
            ...
            craft()->templates->includeJs($formSubmitJs);
        }
        ...
        if ($form->getAnchor() && $form->isPagePosted() && !$form->isValid()) {
            ...
            craft()->templates->includeJs($invalidFormJs);
        }
    }
